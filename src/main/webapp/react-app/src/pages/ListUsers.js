...
const UserCard = ({
  ...
  const getRoleChip = (role) => {
    ...
    return (
      roles[role] || { label: role, color: 'default', icon: <PersonIcon /> }
    );
  };
    ...
    >
      ...
      >
        <Tooltip title="Gerir Conta">
          <IconButton
            size="small"
            onClick={() => onManageAccount(userId)}
            color="primary"
            sx={{ p: 0.5, '&:hover': { color: 'primary.light' } }}
          >
        ...
        <Tooltip
          title={
            isSysAdmin ? 'Não pode alterar um SysAdmin' : 'Alterar Role/Estado'
          }
        >
          <span>
            <IconButton
              size="small"
              onClick={() => onChangeRole(userId)}
              color="secondary"
              disabled={isSysAdmin}
              sx={{ p: 0.5, '&:hover': { color: 'secondary.light' } }}
            >
        ...
        <Tooltip
          title={
            isSysAdmin ? 'Não pode remover um SysAdmin' : 'Remover Utilizador'
          }
        >
          <span>
            <IconButton
...
const ListUsers = () => {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      enqueueSnackbar(
        error.response?.data?.message || 'Erro ao remover utilizador.',
        { variant: 'error' }
      );
    } finally {
        ...
        {paginatedUsers.map((user) => (
          ...
          >
            <UserCard
              user={user}
              onEdit={(userId) =>
                navigate(`/dashboard/change-attributes/${userId}`)
              }
              onChangeRole={(userId) =>
                navigate(`/dashboard/change-role/${userId}`)
              }
              onDelete={handleOpenDeleteDialog}
              onManageAccount={(userId) =>
                navigate(`/dashboard/account-management/${userId}`)
              }
            />
          </Grid>
        ))}
